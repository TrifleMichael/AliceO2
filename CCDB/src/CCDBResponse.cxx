...
int CCDBResponse::countObjects(rapidjson::Document *document) 
{
    auto objectsArray = (*document)["objects"].GetArray();
...
string CCDBResponse::getStringAttribute(rapidjson::Document *document, int ind, string attributeName)
{
    auto objArray = (*document)["objects"].GetArray();
    const char* attrNameChar = attributeName.c_str();
    if ( objArray.Size() <= ind )
    {
        cout << "THIS NEEDS PROPER ERROR HANDLEING\n"; // needs proper error handleing
...
long CCDBResponse::getLongAttribute(rapidjson::Document *document, int ind, string attributeName)
{
    auto objArray = (*document)["objects"].GetArray();
    const char* attrNameChar = attributeName.c_str();
    if ( objArray.Size() <= ind )
...
void CCDBResponse::browse(rapidjson::Document *document)
{
...
void CCDBResponse::latest(rapidjson::Document *document)
{
    browse(document);
