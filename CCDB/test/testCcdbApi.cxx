...
BOOST_AUTO_TEST_CASE(test1)
{
  ...
  api.loadFileToMemory(dest, "/MID/Calib/CTFDictionaryTree", metadata, 1695780534334, &headers, "", "", "");
  std::cout << "Ending " << 1 << "\n";
}
...
BOOST_AUTO_TEST_CASE(test2)
{
  ...
  std::map<std::string, std::string> metadata;
  std::map<std::string, std::string> headers;
  api.loadFileToMemory(dest, "/Task/Detector/1?HTTPOnly=true", metadata, 1, &headers, "", "", "");
  std::cout << "Ending " << 2 << "\n";
}
...
BOOST_AUTO_TEST_CASE(test3)
{
  ...
  std::map<std::string, std::string> metadata;
  std::map<std::string, std::string> headers;
  api.loadFileToMemory(dest, "Task/Detector/1", metadata, 1, &headers, "", "", "");
  std::cout << "Ending " << 3 << "\n";
}
...
//   std::map<std::string, std::string> metadata;
//   std::map<std::string, std::string> headers;
//   api.loadFileToMemory(dest, "/Task/Detector/1", metadata, 0, &headers, "", "", "");
// }
